...
energy_offset = 0.0
ylim_lower = -100000000.
ylim_upper = -100000000.
...
parser.add_option("--Emax", dest="Emax", type="float",
      default=-100000000.,
      help="Maximum energy value on the y axis of the plot(s).")

parser.add_option("--Eoffset", dest="Eoffset", type="float",
      default=0.,
      help="An energy offset that may be aplied to shift the values on the y axis of the plot(s).")

parser.add_option("--legend_x", dest="legend_x", type="float",
      default=1.5,
      help="A x offset on the canvas that allows one to shift the legend horizontally.")

parser.add_option("--legend_y", dest="legend_y", type="float",
      default=1.0,
      help="A y offset on the canvas that allows one to shift the legend vertically.")

parser.add_option("--no_legend", action="store_true", dest="no_legend",help="No legend will be printed into the plot.")

parser.add_option("--show_l_components", action="store_true", dest="show_l",
...
if ylim_lower == -100000000. and ylim_upper == -100000000.:
   ...
   CUSTOM_YLIM = True
   if ylim_lower == -100000000.:
      ylim_lower = -20.
   if ylim_upper == -100000000.:
      ylim_upper = 5.
...
# JKQ: plt.subplot(), originally it was ax_bands = subplot(), cannot find subplot, otherwise from plt import *.
